<?php if ($_SERVER["REQUEST_METHOD"] == "POST") { ?>
	...
	<body>
		<div class="container">
			...
			$subject_items = [];
			$group_headers = [];
			// echo "<pre>"; print_r($subjects_with_group_total2); echo "</pre>";
			...
			<div class="header">
				...
				<div class="school-info">
					<div class="school-name"><?php echo $school_info[0]['school_name']; ?></div>
					<div class="school-details">
						<?php echo $school_info[0]['school_address_line_1'] .  ", Pin " . $school_info[0]['school_pincode'] . ", " . $school_info[0]['school_address_line_2'] ?><br>
						<b>Tel: </b><?php echo $school_info[0]['school_phone']; ?> / <?php echo $school_info[0]['support_phone']; ?><br>
			...
			<div style="width: 100%; display: flex;">
				...
				<table class="subjects-table">
					<thead>
						<tr>
							...
							<th>Total<br>(<?php echo $subjects_with_group_total[0]['max_internal_marks'] + $subjects_with_group_total[0]['ut_max_marks'] + $subjects_with_group_total[0]['max_marks'] ?>)</th>
						</tr>
					</thead>
					<tbody>
						<?php
						...
						?>
								...
								?>">
									...
									<?php echo isset($row['internal_marks_secured']) ? customRound($row['internal_marks_secured']) : '-'; ?>
								</td>
								<td style="text-align: center; padding-left: 10px;">
									<?php echo $row['ut_obtained_marks']; ?>
								</td>
								<td style="text-align: center;">
									<?php echo customRound($row['marks_obtained']); ?>
								</td> -->
									<!-- IA Grade -->

									<?php
									// implement same logic as first snippet
									if ((empty($row['gp_row']) || $row['gp_row'] == 0) && ((isset($row['key_value']) && $row['key_value'] != 1) && isset($row['ut_key_value']) && $row['ut_key_value'] != 1)) {
										// both key_value !=1 and ut_key_value !=1
									...
									?>
										...
										<td style="text-align: center;">
											...
											echo round($total_combined_marks);
											?>
										</td>
										<!-- <td>grade here if needed</td> -->
									...
									?>
										<td style="text-align: center;"><?php echo round($row['internal_marks_secured']); ?></td>
										<td style="text-align: center;"><?php echo $att; ?></td>
										<td style="text-align: center;"><?php echo round($row['marks_obtained']); ?></td>
										<td style="text-align: center;">
											<?php
											$total_combined_marks = $row['internal_marks_secured'] + $row['ut_obtained_marks'] + $row['marks_obtained'];
											echo round($total_combined_marks);
									...
									?>
										<td style="text-align: center;"><?php echo round($row['internal_marks_secured']); ?></td>
										<td style="text-align: center;"><?php echo round($row['ut_obtained_marks']); ?></td>
										<td style="text-align: center;"><?php echo round($row['marks_obtained']); ?></td>
										<td style="text-align: center;">
											<?php
											$total_combined_marks = $row['internal_marks_secured'] + $row['ut_obtained_marks'] + $row['marks_obtained'];
				...
				<table class="subjects-table" style="width: 60%;">
					...
					<tbody>
							...
							if ($row['eval_type'] == 'Graded') { ?>
								<tr>
									...
									?>
										<td>
											<?php
											$graded_max_marks = $row['max_internal_marks'] + $row['ut_max_marks'] + $row['max_marks'];
											$graded_scored_marks = $row['internal_marks_secured'] + $row['ut_obtained_marks'] + $row['marks_obtained'];
											$grade = getGrade2($graded_scored_marks, $graded_max_marks, $grading_scale_2, $grading_scale_2_max_upper_limit);
											echo $grade['disp_value'];
											?>
										</td>
									<?php
									}
						...
						// Grade Table Rows
						foreach ($grading_scale_2 as $grade_row) {
							echo "<tr>";
			...
			$remarks = getRemark($remarks, 'remarks');
			...
			<table class="remarks-table">
				...
				<tbody>
					...
					<tr>
						<td>Participation</td>
						<td><?php echo $remarks['disp_value'] ?></td>
					</tr>
					<tr>
